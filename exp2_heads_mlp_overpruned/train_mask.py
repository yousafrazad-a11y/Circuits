...
def main():
    ...
    parser.add_argument("--dataset", type=str, nargs='+', required=True,
                        help="One or more dataset names (resolved to exp2_heads_mlp_overpruned/datasets/<name>.jsonl) "
    ...
    # Load dataset(s): names resolve to exp2_heads_mlp_overpruned/datasets/<name>.jsonl,
    # or pass direct .jsonl paths. Multiple datasets are mixed and shuffled (seed 42).
    mixed_data = []
    for name in args.dataset:
        path = name if name.endswith(".jsonl") else f"exp2_heads_mlp_overpruned/datasets/{name}.jsonl"
    ...
    os.makedirs("exp2_heads_mlp_overpruned/masks", exist_ok=True)
    mask_path = f"exp2_heads_mlp_overpruned/masks/{args.output_name}_mask.pt"
    ckpt_path = f"exp2_heads_mlp_overpruned/masks/{args.output_name}_checkpoint.pt"
