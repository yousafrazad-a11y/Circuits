"""
Docstring Task - Circuit Discovery Training Script for Llama Models.

Discovers the minimal circuit in Llama that implements the docstring
argument prediction task from the ACDC paper (Conmy et al., NeurIPS 2023).

Usage:
    python docstring_llama.py
    python docstring_llama.py --dry-run
    python docstring_llama.py --model meta-llama/Llama-3.1-8B --lambda-sparsity 0.80
"""
...
from dataset.docstring_llama import (
    DocstringDatasetLlama,
    run_evaluation,
    filter_dataset_by_model_correctness,
)
from dataset.docstring import generate_docstring_data
from utils import disable_dropout, analyze_and_finalize_circuit


# ==============================================================================
# PRUNING CONFIGURATION (tuned for Llama)
# ==============================================================================

@dataclass
class DocstringPruningConfig(PruningConfig):
    """PruningConfig with defaults tuned for Docstring task on Llama."""
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Docstring Circuit Discovery for Llama")
    ...
    parser.add_argument('--lambda-sparsity', type=float, default=0.50,
                        help='Sparsity loss weight')
    parser.add_argument('--hf-token', type=str, default=None, help='HuggingFace API token')
    parser.add_argument('--save-dir', type=str, default='checkpoints_llama_docstring',
    ...
    MAX_SEQ_LEN = 64  # Docstring prompts are ~40-55 tokens
    ...
    # Data sizes
    NUM_TRAIN = 50 if args.dry_run else 5000
    NUM_VAL = 20 if args.dry_run else 500
    NUM_TEST = 20 if args.dry_run else 1000
    ...
    pruning_config = DocstringPruningConfig()
    ...
    train_data = generate_docstring_data(num_samples=NUM_TRAIN, seed=42)
    val_data = generate_docstring_data(num_samples=NUM_VAL, seed=123)
    test_data = generate_docstring_data(num_samples=NUM_TEST, seed=456)
    ...
    train_dataset = DocstringDatasetLlama(train_data, tokenizer, max_length=MAX_SEQ_LEN)
    val_dataset = DocstringDatasetLlama(val_data, tokenizer, max_length=MAX_SEQ_LEN)
    test_dataset = DocstringDatasetLlama(test_data, tokenizer, max_length=MAX_SEQ_LEN)
    ...
    for epoch in epoch_pbar:
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            # Task loss: correct arg should beat distractor
        ...
        if (epoch + 1) % 10 == 0:
            ...
            # Only track best checkpoint AFTER warmup ends
    ...
    # --- Summary ---
    print("\n" + "=" * 60)
    print("FINAL SUMMARY - Docstring Circuit Discovery (Llama)")
    print("=" * 60)
    print(f"Model: {MODEL_NAME}")
    print(f"Lambda sparsity: {LAMBDA_SPARSITY}")
