...
def main():
    ...
    parser.add_argument("--dataset", type=str, nargs='+', required=True,
                        help="One or more dataset names (resolved to exp3_dual_init_union/datasets/<name>.jsonl) "
    ...
    # Load dataset(s): names resolve to exp3_dual_init_union/datasets/<name>.jsonl,
    # or pass direct .jsonl paths. Multiple datasets are mixed and shuffled (seed 42).
    mixed_data = []
    for name in args.dataset:
        path = name if name.endswith(".jsonl") else f"exp3_dual_init_union/datasets/{name}.jsonl"
    ...
    os.makedirs("exp3_dual_init_union/masks", exist_ok=True)
    mask_path = f"exp3_dual_init_union/masks/{args.output_name}_mask.pt"
    ckpt_path = f"exp3_dual_init_union/masks/{args.output_name}_checkpoint.pt"
