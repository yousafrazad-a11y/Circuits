"""
IOI Circuit Discovery Experiment for Llama 3 Family Models.

Mirrors the GPT-2 ioi.py experiment but configured for Llama architecture.
Uses dual-stream (clean/corrupted) forward pass with differentiable pruning
via HardConcreteGates to discover the minimal circuit for the IOI task.

Usage:
    python ioi_llama.py
    python ioi_llama.py --dry-run   # Quick test with minimal samples
"""
...
class LlamaPruningConfig(PruningConfig):
    ...
    # Start with gates FULLY OPEN
    init_value: float = 0.5

    # Warmup: don't prune for first ~1000 steps
    sparsity_warmup_steps: int = 1000

    depth_penalty_scaling: float = 0.0

    # Heads: Llama-1B has 32 heads per layer, 16 layers
    prune_attention_heads: bool = True
    lambda_attention_heads: float = 0.8

    # Hidden neurons: Llama-1B has 8192 intermediate neurons
...
def _save_checkpoint(circuit_model, optimizer, epoch, total_steps,
    ...
    # Extract only gate parameters to keep checkpoint small
    gate_state = {}
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="IOI Circuit Discovery for Llama")
    ...
    parser.add_argument('--save-dir', type=str, default='checkpoints_llama_ioi',
    ...
    # Load with bfloat16 to save memory, then we'll handle mixed precision
    ...
    total_params = 0
    trainable_params = 0
    # Identify HardConcreteGate parameters specifically.
    # IMPORTANT: can't just check 'gate' in name because Llama has 'gate_proj' (SwiGLU).
    # HardConcreteGate params match patterns like: head_gates., neuron_gates., hidden_gates.,
    # output_gates., attention_block_gate., mlp_block_gate., embedding_gate., layer_gates.
    ...
    for name, param in circuit_model.named_parameters():
        ...
        if not is_pruning_gate:
            ...
            # Gates should be in float32 for stable training
    ...
    # Generate IOI data on-the-fly for Llama
    train_data = generate_ioi_data_llama(
        num_samples=NUM_TRAIN, tokenizer=tokenizer, seed=42
    )
    val_data = generate_ioi_data_llama(
        num_samples=NUM_VAL, tokenizer=tokenizer, seed=123
    )
    test_data = generate_ioi_data_llama(
        num_samples=NUM_TEST, tokenizer=tokenizer, seed=456
    )
    ...
    if not args.no_resume:
        ...
        if os.path.exists(SAVE_DIR):
            # Priority: final > latest periodic > best
            candidates = []
    ...
    # --- Pre-cache full model outputs (speedup: eliminates full model forward during training) ---
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(train_dataloader, desc="Caching full model")):
            ...
            cached_train_logits[batch_idx] = out.logits.detach()  # keep on GPU (128K vocab → too large for CPU→GPU transfer)
    ...
    for epoch in epoch_pbar:
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            # Task loss (encourages target over distractor)
        ...
        if (epoch + 1) % 10 == 0:
            ...
            # Also evaluate on test set periodically to catch val/test gap early
            if (epoch + 1) % 50 == 0:
    ...
    # Save final checkpoint (only if training actually ran)
    ...
    # --- Pre-finalization evaluation (diagnose val/test gap) ---
    # This runs BEFORE analyze_and_finalize_circuit modifies gate parameters
    print("\n--- Pre-finalization evaluation on test set (BEFORE hierarchical enforcement) ---")
    circuit_model.eval()
    ...
    # NOTE: This modifies gate log_alpha values in-place for hierarchical
    # consistency (sets pruned gates to -1e6). This can cause accuracy drops
    # compared to pre-finalization evaluation.
    ...
    # --- Also evaluate best checkpoint through the full post-pruning pipeline ---
    ...
    if os.path.exists(best_ckpt_path):
        ...
        # Load best checkpoint gates (overwrites any finalization from above)
        ...
        # Reset final mode so analyze_and_finalize_circuit can set it fresh
        circuit_model.set_final_circuit_mode(False)

        # Pre-finalization eval (before hierarchical enforcement)
        circuit_model.eval()
        ...
        # Run full finalization (hierarchical enforcement, compression analysis)
        print("\n--- Best checkpoint: Analyzing and finalizing circuit ---")
        analyze_and_finalize_circuit(circuit_model)

        # Post-finalization eval
