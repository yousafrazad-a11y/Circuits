...
def main():
    ...
    parser.add_argument("--dataset", type=str, nargs='+', required=True,
                        help="One or more dataset names (resolved to intersection_experiments_4/datasets/<name>.jsonl) "
    ...
    # Load dataset(s): names resolve to intersection_experiments_4/datasets/<name>.jsonl,
    # or pass direct .jsonl paths. Multiple datasets are mixed and shuffled (seed 42).
    mixed_data = []
    for name in args.dataset:
        path = name if name.endswith(".jsonl") else f"intersection_experiments_4/datasets/{name}.jsonl"
    ...
    os.makedirs("intersection_experiments_4/masks", exist_ok=True)
    mask_path = f"intersection_experiments_4/masks/{args.output_name}_mask.pt"
    ckpt_path = f"intersection_experiments_4/masks/{args.output_name}_checkpoint.pt"
