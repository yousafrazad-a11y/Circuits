"""
Two-phase circuit discovery for Llama on the Gender Pronouns (GP) task:
...
Usage (from the circuit_pruning/ directory):
  python -m edge_pruning.train_edge_llama_gp                     # run both phases
  python -m edge_pruning.train_edge_llama_gp --skip-node-pruning # load saved nodes, edge pruning only
"""
...
class LocalNodePruningConfig:
    """Llama node pruning config for GP task."""
...
def run_node_pruning(
    ...
    num_epochs=500, lr=3e-1, lambda_sparsity=0.975,
):
    """Run node-level circuit pruning for the GP task on Llama."""
    print("\n" + "=" * 70)
    print("  PHASE 1: NODE PRUNING (Gender Pronouns — Llama)")
    print("=" * 70)
    ...
    for epoch in pbar:
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            for i in range(bs):
                ...
                total_kl += F.kl_div(
                    F.log_softmax(circuit_out.logits[i, pred_pos].float(), dim=-1),
            ...
            lg = circuit_out.logits[idx, pred_pos, batch["target_token"]].float()
            lb = circuit_out.logits[idx, pred_pos, batch["distractor_token"]].float()
...
def run_edge_pruning(
    ...
    train_dataloader, val_dataloader, test_dataloader,
    device, hf_token=None,
    num_epochs=300, lr=3e-2, lambda_sparsity=0.95,
):
    """Run edge-level circuit pruning on surviving nodes for the GP task on Llama."""
    print("\n" + "=" * 70)
    print("  PHASE 2: EDGE PRUNING (Gender Pronouns — Llama)")
    print("=" * 70)
    ...
    for epoch in pbar:
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            for i in range(bs):
                ...
                total_kl += F.kl_div(
                    F.log_softmax(edge_out.logits[i, pred_pos].float(), dim=-1),
                    F.log_softmax(target_logits[i, pred_pos].float(), dim=-1),
                    reduction="batchmean", log_target=True,
                )
            kl_loss = total_kl / bs
            ...
            idx = torch.arange(bs, device=device)
            pred_pos = batch["prefix_length"] - 1
            lg = edge_out.logits[idx, pred_pos, batch["target_token"]].float()
            lb = edge_out.logits[idx, pred_pos, batch["distractor_token"]].float()
            task_loss = F.relu(0.1 - (lg - lb)).mean()
...
def main():
    parser = argparse.ArgumentParser(description="Node + Edge circuit pruning for Llama on Gender Pronouns")
    ...
    parser.add_argument("--node-checkpoint",
                        default="edge_pruning/active_nodes_llama_gp.json",
    ...
    parser.add_argument("--node-lambda-sparsity", type=float, default=0.5)
    parser.add_argument("--edge-lambda-sparsity", type=float, default=0.9)
    ...
    # -- Dataset --
    print("\nSetting up Gender Pronouns dataset for Llama...")
    ...
    train_dl = DataLoader(GPDatasetLlama(train_data, tokenizer, max_length=args.max_seq_length),
                          batch_size=args.batch_size, shuffle=True)
    val_dl = DataLoader(GPDatasetLlama(val_data, tokenizer, max_length=args.max_seq_length),
                        batch_size=args.batch_size, shuffle=False)
    test_dl = DataLoader(GPDatasetLlama(test_data, tokenizer, max_length=args.max_seq_length),
                         batch_size=args.batch_size, shuffle=False)
    ...
    print("\n" + "=" * W)
    print("  PHASE 1 SUMMARY — NODE PRUNING (Gender Pronouns — Llama)")
    print("=" * W)
    ...
    print("\n" + "=" * W)
    print("  PHASE 2 SUMMARY — EDGE PRUNING (Gender Pronouns — Llama)")
    print("=" * W)
