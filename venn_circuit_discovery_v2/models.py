...
class VennAttention(nn.Module):
    ...
    def forward(
        ...
        bsz_a, seq_len_a = a.shape[:2]
        bsz_b, seq_len_b = b.shape[:2]
        hshape_a = (bsz_a, seq_len_a, self.num_heads, self.head_dim)
        hshape_b = (bsz_b, seq_len_b, self.num_heads, self.head_dim)

        a_h = self._pre_oproj(a, **attn_kwargs_a).view(hshape_a)
        b_h = self._pre_oproj(b, **attn_kwargs_b).view(hshape_b)
        ca_h = self._pre_oproj(corr_a, **attn_kwargs_a).view(hshape_a)
        cb_h = self._pre_oproj(corr_b, **attn_kwargs_b).view(hshape_b)
        ...
        flat_a = (bsz_a, seq_len_a, self.num_heads * self.head_dim)
        flat_b = (bsz_b, seq_len_b, self.num_heads * self.head_dim)
        o = self.attn.o_proj
        return (
            o(a_h.reshape(flat_a)),
            o(b_h.reshape(flat_b)),
            o(ca_h.reshape(flat_a)),
            o(cb_h.reshape(flat_b)),
        )
...
class VennDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        att_a, att_b, att_ca, att_cb = self.attn(
            ln(a), ln(b), ln(corr_a), ln(corr_b), attn_kwargs_a, attn_kwargs_b
        )
...
class LlamaVennCircuit(LlamaForCausalLM):
    ...
    def venn_forward(
        self,
        clean_a_input_ids: torch.LongTensor,
        clean_b_input_ids: torch.LongTensor,
        ...
        position_ids_a: Optional[torch.LongTensor] = None,
        ...
        # All streams are now fully independent.
        a = embed(clean_a_input_ids)
        b = embed(clean_b_input_ids)
        corr_a = embed(corr_a_input_ids).detach()
        corr_b = embed(corr_b_input_ids).detach()

        bsz_a, seq_len_a = a.shape[:2]
        bsz_b, seq_len_b = b.shape[:2]
        device = a.device

        cache_position_a = torch.arange(seq_len_a, device=device)
        cache_position_b = torch.arange(seq_len_b, device=device)
        
        if position_ids_a is None:
            position_ids_a = cache_position_a.unsqueeze(0)
        if position_ids_b is None:
            position_ids_b = cache_position_b.unsqueeze(0)

        pos_emb_a = self._build_position_embeddings(a, position_ids_a)
        pos_emb_b = self._build_position_embeddings(b, position_ids_b)
        
        causal_mask_a = self._build_causal_mask(attention_mask_a, a, cache_position_a)
        causal_mask_b = self._build_causal_mask(attention_mask_b, b, cache_position_b)

        attn_kwargs_a = dict(
            attention_mask=causal_mask_a,
            position_embeddings=pos_emb_a,
            past_key_value=None,
            cache_position=cache_position_a,
        )
        
        attn_kwargs_b = dict(
            attention_mask=causal_mask_b,
            position_embeddings=pos_emb_b,
            past_key_value=None,
            cache_position=cache_position_b,
        )

        for i, layer in enumerate(self.model.layers):
            a, b, corr_a, corr_b = layer(
                a, b, corr_a, corr_b, attn_kwargs_a, attn_kwargs_b
            )
