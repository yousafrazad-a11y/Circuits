...
class HardConcreteGate(nn.Module):
    ...
    def forward(self) -> torch.Tensor:
        ...
        if self.training:
            # Prevent "Death by Adam Epsilon" where heavily pruned gates 
            # fall so deeply negative that the sigmoid gradient vanishes below Adam's eps.
            with torch.no_grad():
                self.log_alpha.clamp_(-5.0, 5.0)
        ...
        # Unconditional STE: Forward pass is strictly binary, backward pass flows directly into s_stretched
        gate_hard = (s_stretched > 0.5).float()
        return (gate_hard - s_stretched).detach() + s_stretched
