...
@dataclass
class VennBatch:
    """A single training batch for two completely independent corrupted tasks.

    ``answer_positions_a``/``b`` indexes the token whose logits are read for the margin
    loss; ``kl_position_mask_a``/``b`` (optional) selects the positions that count toward
    the faithfulness KL.
    """

    clean_a_input_ids: torch.Tensor
    clean_b_input_ids: torch.Tensor
    corr_a_input_ids: torch.Tensor
    corr_b_input_ids: torch.Tensor

    answer_positions_a: torch.Tensor          # (B,)
    answer_positions_b: torch.Tensor          # (B,)
    ...
    attention_mask_a: Optional[torch.Tensor] = None      # (B, Ta)
    attention_mask_b: Optional[torch.Tensor] = None      # (B, Tb)
    
    kl_position_mask_a: Optional[torch.Tensor] = None     # (B, Ta)
    kl_position_mask_b: Optional[torch.Tensor] = None     # (B, Tb)
...
class VennTrainer:
    ...
    def _task_losses(
        ...
        kl_position_mask: Optional[torch.Tensor],
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Return ``(kl, margin)`` for one task's circuit logits."""
        kl = kl_divergence_loss(logits, golden, kl_position_mask)
        mrg = margin_loss(
            logits, target, distractor, answer_positions, margin=self.config.margin
        )
        return kl, mrg
    ...
    def step(self, batch: VennBatch, step_idx: int) -> StepMetrics:
        ...
        # 1. Golden (clean) logits -- the shared faithfulness target for each task.
        golden_a = self.model.golden_logits(batch.clean_a_input_ids, batch.attention_mask_a)
        golden_b = self.model.golden_logits(batch.clean_b_input_ids, batch.attention_mask_b)

        # 2. Four-stream Venn pass -> logits for each task.
        out = self.model.venn_forward(
            clean_a_input_ids=batch.clean_a_input_ids,
            clean_b_input_ids=batch.clean_b_input_ids,
        ...
        # 3. Faithfulness + correctness + accuracy for each task.
        kl_a, margin_a = self._task_losses(
            out.logits_a, golden_a, batch.target_a, batch.distractor_a, batch.answer_positions_a, batch.kl_position_mask_a
        )
        kl_b, margin_b = self._task_losses(
            out.logits_b, golden_b, batch.target_b, batch.distractor_b, batch.answer_positions_b, batch.kl_position_mask_b
        )
        
        ans_logits_a = out.logits_a[torch.arange(out.logits_a.size(0)), batch.answer_positions_a]
        ans_logits_b = out.logits_b[torch.arange(out.logits_b.size(0)), batch.answer_positions_b]
        acc_a = (ans_logits_a.argmax(dim=-1) == batch.target_a).float().mean()
        acc_b = (ans_logits_b.argmax(dim=-1) == batch.target_b).float().mean()
    ...
    def _log(self, epoch: int, m: StepMetrics) -> None:
        print(
            ...
            f"acc_a={m.acc_a:.2f} acc_b={m.acc_b:.2f} | "
