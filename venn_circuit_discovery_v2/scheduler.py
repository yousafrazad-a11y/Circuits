    ...
    * KL above target (too much damage)          -> negative error -> lower lambda.
    ...
    def step(self, measurement: float) -> float:
        ...
        if not self._initialised:
            # Initialize integral so that output starts at init_lambda
            if self.ki > 0:
                self._integral = (self.output - self.kp * error) / self.ki
            self._initialised = True

        self._integral += error
        
        # Direct integral clamping prevents both windup AND back-calculation rebound bugs.
        if self.ki > 0:
            # The integral component (ki * integral) should never push beyond the max 
            # output, nor should it accumulate a massive negative deficit.
            self._integral = _clamp(self._integral, 0.0, self.out_max / self.ki)
        ...
        self.output = _clamp(control, self.out_min, self.out_max)
        return self.output
