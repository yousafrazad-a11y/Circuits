...
Fixes for Dataset 4 errors:
1. Reverts to 6-element noun-only schema. Verb is hardcoded as "is placed in the"
   and later "is moved to the".
2. Enforces physical size hierarchy: mobile container MUST logically fit in outer1,
   and outer1 MUST logically fit in outer2.
3. Explicitly bans surfaces (shelf, table, desk) so "in the" makes grammatical sense.
"""
...
THEMES: dict[str, List[str]] = {
    "The Storage Room": ["coin", "gem", "box", "safe", "cart", "warehouse"],
    "The Treasury": ["gold", "cash", "case", "vault", "truck", "ship"],
    "The Supply Yard": ["brick", "pipe", "bin", "locker", "boxcar", "yard"],
    "The Toy Chest": ["doll", "block", "chest", "trunk", "wagon", "garage"],
}
...
2. `mobile_container`: A small portable ENCLOSURE (e.g., "box", "chest", "case"). NEVER use a surface like "shelf" or "table". Must fit "is placed in the".
3. `fixed_container`: A heavy stationary ENCLOSURE (e.g., "safe", "vault", "locker"). NEVER use a surface. Must fit "is placed in the".
4. `outer_transport`: A large vehicle or movable container (e.g., "cart", "truck", "boxcar"). The `mobile_container` MUST physically fit inside this.
5. `final_destination`: A massive location or building (e.g., "warehouse", "yard", "depot"). The `outer_transport` MUST logically be able to move here.
6. NO BUILDINGS AS TRANSPORTS. You cannot move a "room" or "warehouse".
7. Output strictly a single JSON array and nothing else."""
...
2. SIZE HIERARCHY. The `mobile_container` must fit inside `outer_transport`. The `outer_transport` must be able to move to `final_destination`.
3. NO MOVING BUILDINGS. `outer_transport` cannot be a room, building, or warehouse. It must be a vehicle or large movable bin.
...
def generate_prompts(entities: List[str], theme_name: str) -> dict:
    ...
    clean_p = (
        ...
        f"The {mobile_c} is moved to the {outer1}. "
        f"The {outer1} is moved to the {outer2}. "
...
def main():
    ...
    parser.add_argument("--output", type=str, default="/home/exouser/pruning/datasets/dataset5.jsonl")
