#!/usr/bin/env python3
"""
6-Hop State-Machine Dataset Generator  (single 40GB A100, one model at a time)
==============================================================================
...
THEMES: dict[str, List[str]] = {
    "The Playroom": [
        "ball", "block", "bag", "box",
        "baby", "boy", "girl", "mom", "dad", "friend",
    ],
    "The Kitchen": [
        "grape", "bean", "bowl", "pan",
        "cook", "chef", "host", "guest", "kid", "wife",
    ],
    "The Forest": [
        "rock", "stick", "bucket", "nest",
        "fox", "bear", "wolf", "deer", "hawk", "owl",
    ],
    "The Pets": [
        "bone", "yarn", "pouch", "jar",
        "pug", "cat", "bird", "mouse", "frog", "duck",
    ],
    "The School": [
        "book", "pen", "pack", "desk",
        "kid", "child", "teen", "coach", "nurse", "guard",
    ],
    "The Beach": [
        "shell", "coin", "net", "chest",
        "crab", "fish", "seal", "shark", "whale", "gull",
    ],
    "The Farm": [
        "seed", "leaf", "sack", "pot",
        "pig", "cow", "horse", "goat", "sheep", "mule",
    ],
    "The Bug Garden": [
        "star", "cube", "red cup", "blue cup",
        "ant", "bee", "moth", "worm", "fly", "snail",
    ],
    "Simple Jobs": [
        "sock", "hat", "case", "tray",
        "cop", "vet", "clerk", "smith", "mayor", "judge",
    ],
    "Simple Fairy Tale": [
        "ring", "gem", "purse", "safe",
        "elf", "troll", "witch", "ghost", "king", "queen",
    ],
}
...
THEME_VARIATIONS: dict[str, List[str]] = {
    "The Playroom": ["ball", "top", "doll", "drum", "kite", "car"],
    "The Kitchen": ["grape", "bean", "egg", "plum", "pea", "nut"],
    "The Forest": ["rock", "stick", "cone", "leaf", "twig", "moss"],
    "The Pets": ["bone", "yarn", "ball", "sock", "toy", "treat"],
    "The School": ["book", "pen", "note", "chalk", "card", "clip"],
    "The Beach": ["shell", "coin", "ball", "stone", "star", "clam"],
    "The Farm": ["seed", "leaf", "egg", "corn", "hay", "nut"],
    "The Bug Garden": ["star", "cube", "bead", "gem", "coin", "chip"],
    "Simple Jobs": ["sock", "hat", "key", "pen", "coin", "ring"],
    "Simple Fairy Tale": ["ring", "gem", "coin", "crown", "wand", "key"],
}
...
# ---------------------------------------------------------------------------
INTERP_REQUIREMENTS = (
    "The data is a list of 10 elements in this exact order:\n"
    "  [1] target_item     - the object the question asks about\n"
    "  [2] distractor_item - a second, different object\n"
    "  [3] mobile_container - a container that is handed from one to the next\n"
    "  [4] fixed_container  - a container that stays put (the 1-hop trap)\n"
    "  [5..10] six roles    - six DIFFERENT LIVING agents (people or animals), "
    "in a sensible order, who pass the mobile container one to the next\n\n"
    "They are dropped into this fixed frame:\n"
    "  'The <item> is put in the <container>. ... The <mobile> is given to the "
    "<role1>. The <role1> hands it to the <role2>. ... The <item> is held by "
    "the <role6 OR container>.'\n\n"
    "Approve when ALL of these hold (be reasonable, not nitpicky -- reject only "
    "on a clear, specific problem):\n"
    "1. TODDLER-SIMPLE WORDS: THIS IS THE MOST IMPORTANT RULE. Every element must "
    "be a fundamental, extremely common 1-2 syllable noun that a small child "
    "knows (e.g. ball, cup, bag, box, dog, cat, boy, mom, king). REJECT any word "
    "that is technical, formal, rare, abstract, multi-syllable, or jargon "
    "(e.g. 'Appraiser', 'Curator', 'Operator', 'capsule', 'manuscript', "
    "'vaccine') -- such words add semantic noise that breaks interpretability on "
    "a small model.\n"
    "2. DISTINCT: all 10 strings are different from each other, with no two that "
    "obviously mean the same thing.\n"
    "3. RIGHT KINDS: the two items are small physical objects; the mobile "
    "container can be carried/handed along; the fixed container is one that "
    "naturally stays put; the six roles are LIVING agents (people or animals) "
    "that can hold a thing and hand it on.\n"
    "4. BOTH ITEMS FIT BOTH CONTAINERS: each item could really be put inside "
    "EITHER container, so swapping them still reads naturally (this is what makes "
    "the clean/corrupted pair valid).\n"
    "5. SENSIBLE CHAIN: the six roles make a natural little hand-off sequence; "
    "each 'hands it to the' next one reads fine, and 'is held by the' last role "
    "(a living agent) and 'is held by the' fixed container both make sense.\n"
    "6. CLEAN FORMATTING: each element is 1-2 plain words, lowercase objects / "
    "simple role words, normal spaces, no articles, no sentences, and no "
    "code-style tokens (no underscores / camelCase)."
)

# ---- Generator prompt -----------------------------------------------------
GEN_SYSTEM_TEMPLATE = (
    "You are a strict data-generation engine for a mechanistic-interpretability "
    "dataset. You output ONLY a single JSON array of EXACTLY 10 strings — no "
    "prose, no keys, no markdown, no backticks.\n\n"
    "The 10 strings MUST be in this exact order:\n"
    ...
    "  'The <target_item> is put in the <mobile_container>. "
    "The <distractor_item> is put in the <fixed_container>. "
    "The <mobile_container> is given to the <role1>. The <role1> hands it to "
    "the <role2>. ... The <role5> hands it to the <role6>. "
    "The <target_item> is held by the ...'\n"
    "So: the mobile_container is handed along the roles; the fixed_container "
    "stays put; BOTH items must fit inside BOTH containers; and the six roles "
    "are LIVING agents (people or animals) that can hold a thing and hand it on.\n\n"
    "USE TODDLER-SIMPLE WORDS ONLY: every one of the 10 strings must be a basic "
    "1-2 syllable noun a small child knows (ball, cup, bag, box, dog, cat, boy, "
    "mom, king). Never use technical, formal, or rare words.\n\n"
    "Follow EVERY requirement:\n"
    f"{INTERP_REQUIREMENTS}\n\n"
    "Theme to follow: {theme_name}.\n"
    "STYLE example only (invent a completely different scenario; do NOT reuse "
    "its names): {example}\n"
    "Output ONLY the raw JSON array of 10 strings."
)
GEN_USER_TEMPLATE = (
    "Generate ONE brand-new, original 10-element JSON array for the theme "
...
VERIFY_SYSTEM = (
    ...
    "given a THEME and a 10-element array "
    "[target_item, distractor_item, mobile_container, fixed_container, "
    "role1..role6]. This data drives a clean/corrupted activation-patching pair, "
...
# ---------------------------------------------------------------------------
_CHAIN_TEMPLATE = (
    " The {mobile} is given to the {r1}. The {r1} hands it to the {r2}. "
    "The {r2} hands it to the {r3}. The {r3} hands it to the {r4}. "
    "The {r4} hands it to the {r5}. The {r5} hands it to the {r6}. "
    "The {target} is held by the"
)
CLEAN_TEMPLATE = (
    "The {target} is put in the {mobile}. "
    "The {distractor} is put in the {fixed}." + _CHAIN_TEMPLATE
)
CORRUPTED_TEMPLATE = (
    "The {distractor} is put in the {mobile}. "
    "The {target} is put in the {fixed}." + _CHAIN_TEMPLATE
)
...
def is_structurally_valid(arr: object) -> bool:
    """Local structural gate (run before the judge) for the 10-element schema:
        - exactly 10 non-empty strings,
        - SIMPLE WORDING: each element is 1-2 whitespace-separated words,
        - all 10 elements are mutually distinct (case-insensitive).
    ...
    if len(set(lowered)) != 10:                          # all 10 distinct
        return False
    return True
...
def format_record(theme_name: str, arr: List[str]) -> dict:
    ...
    Dual-chain minimal pair:
      clean     -> target item rides the mobile container 6 hops to role6.
      corrupted -> target item is placed in the fixed container and stays.
    The two targets DIFFER (role6 vs fixed container). Note the leading space on
    the targets (they continue "...currently in the").
    """
    target, distractor, mobile, fixed, r1, r2, r3, r4, r5, r6 = arr
...
def _add_common(p: argparse.ArgumentParser) -> None:
    p.add_argument("--output", type=str, default="6_hop_state_machine.jsonl")
    p.add_argument("--rejected-output", type=str,
                   default="6_hop_state_machine.rejected.jsonl")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="6-hop interpretability dataset on a single 40GB A100 via "
