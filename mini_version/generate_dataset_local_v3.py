...
Outputs:
    draft_chains_v3.jsonl                  pending raw drafts awaiting the judge
...
in draft_chains_v3.jsonl are judged first before more are generated.
"""
...
THEMES: dict[str, List[str]] = {
    "The Storage Room": [
        "coin", "gem", "is placed in the", "chest", "is placed in the", "safe", "truck", "train"
    ],
    "The Treasury": [
        "gold", "cash", "is placed in the", "case", "is placed in the", "vault", "cart", "ship"
    ],
    "The Supply Yard": [
        "brick", "pipe", "is placed in the", "bin", "is placed on the", "pallet", "truck", "yard"
    ],
    "The Toy Chest": [
        "doll", "block", "is placed in the", "case", "is placed in the", "trunk", "wagon", "plane"
    ],
}
...
# ---------------------------------------------------------------------------
INTERP_REQUIREMENTS = (
    "The data is a list of 8 elements in this exact order:\n"
    ...
    "  'The <item> <verb_phrase> <container>. ... The <mobile> is moved to the "
    ...
    "9. GRAMMAR AND PREPOSITIONS: The verb phrases must perfectly match the container conceptually and grammatically. You must use 'on the' for surfaces (e.g. shelf, table, rack) and 'in the' for enclosures (e.g. crate, drawer). REJECT any chain that uses unnatural English like 'is placed in the shelf'."
)

# ---- Generator prompt -----------------------------------------------------
GEN_SYSTEM_TEMPLATE = (
    "You are a strict data-generation engine for a mechanistic-interpretability "
    "dataset. You output ONLY a single JSON array of EXACTLY 8 strings — no "
    "prose, no keys, no markdown, no backticks.\n\n"
    "The 8 strings MUST be in this exact order:\n"
    "[target_item, distractor_item, mobile_verb_phrase, mobile_container, fixed_verb_phrase, fixed_container, "
    "outer1, outer2]\n\n"
    "These strings are dropped into this fixed sentence frame:\n"
    "  'The <target_item> <mobile_verb_phrase> <mobile_container>. "
    "The <distractor_item> <fixed_verb_phrase> <fixed_container>. "
    ...
    "Output ONLY the raw JSON array of 8 strings."
)
GEN_USER_TEMPLATE = (
    "Generate ONE brand-new, original 8-element JSON array for the theme "
...
VERIFY_SYSTEM = (
    ...
    "given a THEME and an 8-element array "
    "[target_item, distractor_item, mobile_verb_phrase, mobile_container, fixed_verb_phrase, fixed_container, "
...
CLEAN_TEMPLATE = (
    "The {target} {mobile_verb} {mobile}. "
    "The {distractor} {fixed_verb} {fixed}." + _CHAIN_TEMPLATE
)
CORRUPTED_TEMPLATE = (
    "The {distractor} {mobile_verb} {mobile}. "
    "The {target} {fixed_verb} {fixed}." + _CHAIN_TEMPLATE
)
...
def is_structurally_valid(arr: object) -> bool:
    """Local structural gate (run before the judge) for the 8-element schema:
        - exactly 8 non-empty strings,
        - SIMPLE WORDING: items and containers 1-2 words, verb phrases 2-5 words.
        - all entities are mutually distinct (case-insensitive).
    ...
    for i, s in enumerate(arr):
        ...
        if i in [2, 4]: # The verb phrases
            if not (2 <= num_words <= 6):
                return False
        else: # The items / containers
            if not (1 <= num_words <= 2):
                return False
                
    lowered = [s.strip().lower() for s in arr]
    # Note: verbs might not be distinct from each other ("is placed in the").
    # We should only check distinctness for items/containers (indices 0, 1, 3, 5, 6, 7)
    entities_only = [lowered[i] for i in [0, 1, 3, 5, 6, 7]]
    if len(set(entities_only)) != 6:
        return False
...
def format_record(theme_name: str, arr: List[str]) -> dict:
    ...
    """
    target, distractor, mobile_verb, mobile, fixed_verb, fixed, r1, r2 = arr
    fields = dict(target=target, distractor=distractor, mobile_verb=mobile_verb, mobile=mobile, fixed_verb=fixed_verb, fixed=fixed,
                  r1=r1, r2=r2)
    return {
        "entities": list(arr),
...
def _add_common(p: argparse.ArgumentParser) -> None:
    p.add_argument("--output", type=str, default="mini_version/dataset3_v3_logic.jsonl")
    p.add_argument("--rejected-output", type=str, default="mini_version/dataset3_v3_logic.rejected.jsonl")
    p.add_argument("--draft-output", type=str, default="mini_version/draft_chains_v3.jsonl")
    ...
    p.add_argument("--gpu-memory-utilization", type=float, default=0.7,
                   help="GPU memory util (reduce if you get OOMs)")
