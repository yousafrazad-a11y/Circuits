...
def evaluate(model, tokenizer, cat_name):
    ...
    print(f"Evaluating {cat_name.upper()}...")
    ...
    for idx, item in enumerate(data):
        ...
        # If target word is anywhere in the first few generated tokens, we consider it a pass
...
def main():
    ...
    for cat_name in CATEGORIES.keys():
        circuit_path = f"/home/exouser/pruning/intersection_experiments/results_5way/{cat_name}_circuit.pt"
        
        if not os.path.exists(circuit_path):
            print(f"Skipping {cat_name}: Circuit not found.")
            continue
            
        print(f"\n--- Loading {cat_name} circuit ---")
        mask = torch.load(circuit_path, weights_only=True)
        
        # Force model gates into this exact mask
        ...
        # Evaluate
        prob_acc, gen_acc = evaluate(model, tokenizer, cat_name)
