"""
Two-phase circuit discovery for GPT-2 on the IOI task:
...
Usage (from the circuit_pruning/ directory):
  python -m edge_pruning.train_edge                     # run both phases
  python -m edge_pruning.train_edge --skip-node-pruning # load saved nodes, edge pruning only
"""
...
class GPUMemoryTracker:
    ...
    Each snapshot stores:
        allocated  – tensors currently held by PyTorch
        reserved   – memory pool held by the CUDA caching allocator
        peak_alloc – high-water mark of allocated since last reset
    """
    ...
    def snap(self, tag: str):
        """Take a snapshot and print a one-liner."""
    ...
    def print_report(self):
        """Print a full memory map with deltas between consecutive snapshots."""
        ...
        # Find phase boundaries by tag keywords
        node_snaps = [(t, a, r, p) for t, a, r, p in self.snapshots if "node" in t.lower()]
        edge_snaps = [(t, a, r, p) for t, a, r, p in self.snapshots if "edge" in t.lower()]

        # Peak during node pruning (snapshots between "Node model loaded" and "Node pruning done")
        node_peaks = [p for t, a, r, p in self.snapshots
                      if "node" in t.lower()]
        edge_peaks = [p for t, a, r, p in self.snapshots
                      if "edge" in t.lower()]
...
# ==============================================================================
# NODE PRUNING CONFIG (mirrors ioi_fast.py — edit here to match your setup)
# ==============================================================================
...
def run_node_pruning(
    ...
    device, num_epochs=500, lr=3e-2, lambda_sparsity=0.99,
):
    """Run node-level circuit pruning (same logic as ioi_fast.py)."""
    print("\n" + "=" * 70)
    print("  PHASE 1: NODE PRUNING (IOI)")
    print("=" * 70)
    ...
    for epoch in pbar:
        ...
        for batch in train_dataloader:
            ...
            for i in range(bs):
                ...
                circuit_logits = circuit_out.logits[i, t_s:end]
                target_logits = target_out.logits[i, t_s:end]
                if t_s < end:
                    total_kl += F.kl_div(
            ...
            lg = circuit_out.logits[idx, pos_good, tok_good]
            lb = circuit_out.logits[idx, pos_bad, tok_bad]
...
def run_edge_pruning(
    ...
    """Run edge-level circuit pruning on surviving nodes."""
    print("\n" + "=" * 70)
    print("  PHASE 2: EDGE PRUNING (IOI)")
    print("=" * 70)
    ...
    for epoch in pbar:
        ...
        for batch in train_dataloader:
            ...
            for i in range(bs):
                ...
                circuit_logits = edge_out.logits[i, t_s:end]
                target_logits = target_out.logits[i, t_s:end]
                if t_s < end:
                    total_kl += F.kl_div(
                        F.log_softmax(circuit_logits, dim=-1),
            ...
            tok_good = batch["target_tokens"][:, 0]
            tok_bad = batch["distractor_tokens"][:, 0]
            lg = edge_out.logits[idx, pos_good, tok_good]
            lb = edge_out.logits[idx, pos_bad, tok_bad]
            task_loss = F.relu(4.0 - (lg - lb)).mean()
...
def main():
    parser = argparse.ArgumentParser(description="Node + Edge circuit pruning for GPT-2 on IOI")
    ...
    parser.add_argument("--node-checkpoint",
                        default="edge_pruning/active_nodes.json",
    ...
    parser.add_argument("--edge-lambda-sparsity", type=float, default=0.99)
    ...
    train_data = load_or_generate_ioi_data(split="train", num_samples=args.train_samples)
    val_data = load_or_generate_ioi_data(split="validation", num_samples=args.val_samples)
    test_data = load_or_generate_ioi_data(split="test", num_samples=args.test_samples)
    ...
    train_dl = DataLoader(IOIDataset(train_data, tokenizer),
                          batch_size=args.batch_size, shuffle=False)
    val_dl = DataLoader(IOIDataset(val_data, tokenizer),
                        batch_size=args.batch_size, shuffle=False)
    test_dl = DataLoader(IOIDataset(test_data, tokenizer),
                         batch_size=args.batch_size, shuffle=False)
    ...
    print("\n" + "=" * W)
    print("  PHASE 1 SUMMARY — NODE PRUNING (IOI)")
    print("=" * W)
    ...
    print("\n" + "=" * W)
    print("  PHASE 2 SUMMARY — EDGE PRUNING (IOI)")
    print("=" * W)
