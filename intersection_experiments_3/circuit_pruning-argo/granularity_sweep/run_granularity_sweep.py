"""
Granularity Ablation Sweep: test each pruning granularity individually on IOI.
...
Fixed: task=IOI, lambda_sparsity=0.93

Outputs:
  - granularity_sweep/results/granularity_results.csv
  - granularity_sweep/results/granularity_results.json
  - granularity_sweep/results/per_epoch/

Usage:
  python -m granularity_sweep.run_granularity_sweep
  python -m granularity_sweep.run_granularity_sweep --granularities attention_heads mlp_blocks
"""
...
# All granularities that are prunable=True in the IOI config
ALL_GRANULARITIES = [
    "attention_heads",
    "mlp_hidden",
    "mlp_output",
...
# IOI training config (mirrors ioi_fast.py exactly)
IOI_CONFIG = {
    "num_epochs": 500,
    "learning_rate": 3e-2,
    "batch_size": 32,
...
def build_pruning_config(granularity: str) -> PruningConfig:
    ...
    If granularity == "all", enable everything (the default IOI setup).
    """
...
# =============================================================================
# DATA LOADING (IOI only)
# =============================================================================

def load_ioi_data(tokenizer, full_model, device, batch_size):
...
# =============================================================================
# IOI TRAINING STEP (mirrors ioi_fast.py exactly)
# =============================================================================
...
def extract_pruning_stats(circuit_model, total_steps):
    ...
    # Always report all granularities with consistent keys.
    # If a granularity wasn't enabled (total==0), report 0% pruned.
...
def run_single_experiment(granularity: str, device: str, data_cache: dict, seed: int = 42):
    """
    Train IOI circuit with only one granularity enabled.
    data_cache contains pre-loaded data loaders to avoid re-loading each run.
    """
    torch.manual_seed(seed)

    print(f"\n{'='*80}")
    print(f"  GRANULARITY ABLATION: {GRANULARITY_LABELS.get(granularity, granularity)}")
    print(f"  lambda_sparsity={LAMBDA_SPARSITY}, task=IOI")
    print(f"{'='*80}")
    ...
    # Build pruning config for this granularity
    ...
    optimizer = AdamW(gate_params, lr=IOI_CONFIG["learning_rate"])
    scheduler = CosineAnnealingLR(optimizer, T_max=IOI_CONFIG["num_epochs"], eta_min=1e-4)
    ...
    epoch_pbar = tqdm(range(IOI_CONFIG["num_epochs"]), desc=f"gran={granularity}")
    ...
    for epoch in epoch_pbar:
        ...
        for batch in train_loader:
            ...
            kl_loss, task_loss = train_step_ioi(circuit_model, full_model, batch, device)
    ...
    result = {
        ...
        "task": "ioi",
        "seed": seed,
        "num_epochs": IOI_CONFIG["num_epochs"],
...
def write_epoch_csv(granularity, epoch_records, dirpath):
    ...
    filepath = os.path.join(dirpath, f"ioi_{granularity}_epochs.csv")
...
def main():
    parser = argparse.ArgumentParser(description="Granularity ablation sweep on IOI")
    parser.add_argument(
        "--granularities",
        nargs="+",
    ...
    print("\n--- Loading IOI data (shared across all granularity runs) ---")
    ...
    train_loader, val_loader, test_loader, run_evaluation = load_ioi_data(
        tokenizer, full_model, device, IOI_CONFIG["batch_size"]
    )
    ...
    # Final summary
    print(f"\n{'='*140}")
    print("GRANULARITY ABLATION COMPLETE - SUMMARY")
    print(f"{'='*140}")
    header = (
