...
def main():
    ...
    parser.add_argument("--dataset", type=str, nargs='+', required=True,
                        help="One or more dataset names (resolved to intersection_experiments_3/datasets/<name>.jsonl) "
    ...
    parser.add_argument("--gate_init", choices=["open", "closed"], default="open",
                        help="Gate log_alpha initialization. open: uniform(2.5, 3.5), the default — gates start "
                             "open and training prunes (Ns run). closed: uniform(-3.5, -2.5) — gates start closed "
                             "and training must open what it needs (Dn run).")
    args = parser.parse_args()

    device = "cuda" if torch.cuda.is_available() else "cpu"
    
    # Load dataset(s): names resolve to intersection_experiments_3/datasets/<name>.jsonl,
    # or pass direct .jsonl paths. Multiple datasets are mixed and shuffled (seed 42).
    mixed_data = []
    for name in args.dataset:
        path = name if name.endswith(".jsonl") else f"intersection_experiments_3/datasets/{name}.jsonl"
    ...
    if args.gate_init == "closed":
        from pruning_manager import HardConcreteGate
        with torch.no_grad():
            n_gates = 0
            for m in manager.model.modules():
                if isinstance(m, HardConcreteGate):
                    m.log_alpha.uniform_(-3.5, -2.5)
                    n_gates += m.log_alpha.numel()
        print(f"Gate init CLOSED: re-initialized {n_gates} gate values ~ U(-3.5, -2.5).")
    else:
        print("Gate init OPEN (default U(2.5, 3.5)).")
    ...
    os.makedirs("intersection_experiments_3/masks", exist_ok=True)
    mask_path = f"intersection_experiments_3/masks/{args.output_name}_mask.pt"
    ckpt_path = f"intersection_experiments_3/masks/{args.output_name}_checkpoint.pt"
