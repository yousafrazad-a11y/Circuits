"""IOI data with eight logical, variable-length position sections.
...
def load_or_generate_ioi_data(
    ...
    """Load a saved DatasetDict when available; otherwise generate aligned IOI data."""
...
class IOIDataset(Dataset):
    """Tokenized IOI prompts with eight variable-length logical sections."""
...
def run_evaluation(
    ...
    with torch.no_grad():
        for batch in iterator:
            ...
            logit_diff = (
                prediction_logits[indices, targets]
                - prediction_logits[indices, distractors]
            )
            totals["correct"] += (logit_diff >= 0).sum().item()
            totals["logit_diff"] += logit_diff.sum().item()
            ...
            if full_model_for_faithfulness is not None:
                ...
                full_logit_diff = (
                    full_prediction_logits[indices, targets]
                    - full_prediction_logits[indices, distractors]
                )
                totals["full_logit_diff"] += full_logit_diff.sum().item()
                totals["kl"] += F.kl_div(
                ...
                totals["exact"] += (
                    ...
                    == full_prediction_logits.argmax(dim=-1)
                ).sum().item()

    count = max(totals["samples"], 1.0)
    average_logit_diff = totals["logit_diff"] / count
    average_full_logit_diff = totals["full_logit_diff"] / count
    results = {
        "accuracy": totals["correct"] / count,
        "logit_diff": average_logit_diff,
        "kl_div": totals["kl"] / count,
        "exact_match": totals["exact"] / count,
        "soft_faithfulness": (
            average_logit_diff / average_full_logit_diff
            if full_model_for_faithfulness is not None
            and abs(average_full_logit_diff) > 1e-12
            else 0.0
        ),
    }
    if verbose:
