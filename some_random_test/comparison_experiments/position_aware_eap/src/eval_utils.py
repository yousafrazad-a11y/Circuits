...
def run_edges_with_mean_ablation(model: HookedTransformer,
    ...
    shared_metrics = CircuitMetricAccumulator()
    model.reset_hooks()
    faithfulness_list, acc_correct_list, task_accuracy_list, kl_divergence_list, avg_circuit_size_list, circuit_prediction_list, model_prediction_list = [],[],[],[],[],[],[]
    ...
    for index, row in tqdm(test_df.iterrows(), total=test_df.shape[0]):
        ...
        correct_id = model.to_single_token(row["correct_token"])
        circuit_final = circuit_logits[0, -1]
        clean_final = clean_logits[0, -1]
        wrong_id = model.to_single_token(row["wrong_token"])
        shared_metrics.update(
            circuit_final, clean_final,
            torch.tensor([correct_id], device=circuit_final.device),
            torch.tensor([wrong_id], device=circuit_final.device),
        )
        task_accuracy_list.append(int(circuit_final.argmax().item() == correct_id))
        kl_divergence_list.append(torch.nn.functional.kl_div(
            circuit_final.log_softmax(dim=-1),
            clean_final.log_softmax(dim=-1),
            reduction="sum",
            log_target=True,
        ).item())
    ...
    result = (faithfulness_list, acc_correct_list, task_accuracy_list,
              kl_divergence_list, avg_circuit_size_list,
              circuit_prediction_list, model_prediction_list)
    return result + (shared_metrics.compute(),) if return_shared_metrics else result
