...
def run_faithfulness(eval_size: int, peap_results_path: str, save_path: str, sum_span_scores:bool, exp: Experiament, top_k: List[int], graph_with_pos: bool, ablation_size: int, search_type: Literal["abs","max","min"], is_reversed: bool, patch_q: bool) -> None:
    ...
    for k in top_k:
        ...
        if len(circuit.edges) == 0:
            continue
        top_k_used.append(k)
        circuit_path = output_path.with_name(
            f"{output_path.stem}.topk_{k}.circuit.pkl"
        )
        with circuit_path.open("wb") as circuit_file:
            pickle.dump({
                "top_k": k,
                "with_positions": graph_with_pos,
                "span_type": span_type,
                "search_type": search_type,
                "circuit": circuit,
            }, circuit_file, protocol=pickle.HIGHEST_PROTOCOL)

        evaluation_output = run_edges_with_mean_ablation(
            model=model, test_df=test_df, abstract_circuit=circuit, exp=exp,
            with_pos=graph_with_pos, sample_ablation_size=ablation_size,
            patch_q=patch_q, return_shared_metrics=True,
        )
        (diff_list, acc_list, task_accuracy_list, kl_divergence_list,
         mean_circuit_size_list, circuit_prediction_list,
         model_prediction_list, shared_metrics) = evaluation_output
        ...
        metrics_path = Path(save_path).with_suffix(".metrics.json")
        metric_rows.append({
            "method": "peap" if graph_with_pos else "eap",
            "top_k": k,
            "n_examples": len(diff_list),
            "accuracy": float(np.mean(task_accuracy_list)),
            "pairwise_accuracy": shared_metrics["pairwise_accuracy"],
            "exact_match_to_full_model": float(np.mean(acc_list)),
            "logit_diff": float(np.mean(diff_list)),
            "full_model_logit_diff": float(model_performance),
            "soft_faithfulness": float(np.mean(diff_list) / model_performance) if abs(model_performance) > 1e-12 else 0.0,
            "kl_div": float(np.mean(kl_divergence_list)),
            "mean_circuit_edges": float(np.mean(mean_circuit_size_list)),
        })
        metrics_path.write_text(json.dumps(metric_rows, indent=2, sort_keys=True))
