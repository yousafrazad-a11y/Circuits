...
def _atomic_pickle_dump(value, output_path: Path) -> None:
    """Write a pickle atomically so an interrupted write cannot corrupt it."""
...
# Select the same rows from both datasets
    ...
    recovery_path = output_path.with_suffix(output_path.suffix + ".partial")
    for processed_count, (index, row) in enumerate(
        tqdm(ds_clean.iterrows(), total=ds_clean.shape[0]), start=1
    ):
        ...
        # Save raw scores periodically and after the final example.
        if processed_count % 50 == 0 or processed_count == ds_clean.shape[0]:
            _atomic_pickle_dump(results, recovery_path)
            print(f"RECOVERY CHECKPOINT {processed_count}/{ds_clean.shape[0]} {recovery_path}", flush=True)


    # Get the average scores for each edge
    results.get_average_scores()
    
    _atomic_pickle_dump(results, output_path)
    print(f"SAVED PEAP SCORES {output_path}", flush=True)
