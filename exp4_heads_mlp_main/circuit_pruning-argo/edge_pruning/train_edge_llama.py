"""
Two-phase circuit discovery for Llama on the IOI task:
...
Usage (from the circuit_pruning/ directory):
  python -m edge_pruning.train_edge_llama                     # run both phases
  python -m edge_pruning.train_edge_llama --skip-node-pruning # load saved nodes, edge pruning only
"""
...
from edge_pruning.models.llama_edge_circuit import EdgePrunableLlama, EdgePruningConfig
...
# ==============================================================================
# NODE PRUNING CONFIG (matches ioi_llama.py LlamaPruningConfig)
# ==============================================================================

PRUNING_FACTOR = 0.25

# HardConcreteGate parameter patterns (avoids matching Llama's gate_proj in SwiGLU)
GATE_PATTERNS = ('_gates.', '_gate.', 'embedding_gate.', 'layer_gates.')


@dataclass
class LocalNodePruningConfig:
    """Llama node pruning config matching ioi_llama.py defaults."""
...
def run_node_pruning(
    ...
    device, hf_token=None,
    num_epochs=500, lr=3e-2, lambda_sparsity=0.90,
):
    """Run node-level circuit pruning for the IOI task on Llama."""
    print("\n" + "=" * 70)
    print("  PHASE 1: NODE PRUNING (IOI — Llama)")
    print("=" * 70)
    ...
    # Freeze base, unfreeze gates (using patterns to avoid gate_proj)
    ...
    # Pre-cache full model outputs for efficiency
    print("Pre-caching full model outputs for training data...")
    cached_logits = {}
    with torch.no_grad():
    ...
    print(f"Cached {len(cached_logits)} batches.")
    ...
    for epoch in pbar:
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            target_logits = cached_logits[batch_idx]

            # KL loss at T_Start - 1 (batchmean normalizes over 128K vocab)
            ...
            for i in range(bs):
                ...
                if t_s < end:
                    total_kl += F.kl_div(
                        F.log_softmax(circuit_out.logits[i, t_s].float(), dim=-1),
                        F.log_softmax(target_logits[i, t_s].float(), dim=-1),
            ...
            lg = circuit_out.logits[idx, pos_good, tok_good].float()
            lb = circuit_out.logits[idx, pos_bad, tok_bad].float()
    ...
    # Finalize (Llama model doesn't have set_pruning_config — analyze handles it)
    node_stats = analyze_node_circuit(circuit_model)

    # Clean up cached logits
    del cached_logits
...
def run_edge_pruning(
    ...
    """Run edge-level circuit pruning on surviving nodes for the IOI task on Llama."""
    print("\n" + "=" * 70)
    print("  PHASE 2: EDGE PRUNING (IOI — Llama)")
    print("=" * 70)
    ...
    edge_model = EdgePrunableLlama.from_pretrained_with_edges(
        model_name, active_heads, active_mlps, edge_config, **model_kwargs
    ).to(device)
    disable_dropout(edge_model)

    # Only edge gates are trainable (base model already frozen by from_pretrained_with_edges)
    # Ensure gate params are float32
    ...
    for epoch in pbar:
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            for i in range(bs):
                ...
                if t_s < end:
                    total_kl += F.kl_div(
                        F.log_softmax(edge_out.logits[i, t_s].float(), dim=-1),
                        F.log_softmax(target_logits[i, t_s].float(), dim=-1),
            ...
            tok_good = batch["target_tokens"][:, 0]
            tok_bad = batch["distractor_tokens"][:, 0]
            lg = edge_out.logits[idx, pos_good, tok_good].float()
            lb = edge_out.logits[idx, pos_bad, tok_bad].float()
            task_loss = F.relu(4.0 - (lg - lb)).mean()
...
def main():
    parser = argparse.ArgumentParser(description="Node + Edge circuit pruning for Llama on IOI")
    ...
    parser.add_argument("--node-checkpoint",
                        default="edge_pruning/active_nodes_llama_ioi.json",
    ...
    parser.add_argument("--node-lambda-sparsity", type=float, default=0.50)
    parser.add_argument("--edge-lambda-sparsity", type=float, default=0.50)
    ...
    full_model = LlamaForCausalLM.from_pretrained(args.model, **model_kwargs).to(DEVICE).eval()
    for p in full_model.parameters():
        p.requires_grad = False
    _tracker.snap("Full model loaded")

    # -- Dataset --
    print("\nSetting up IOI dataset for Llama...")
    train_data = generate_ioi_data_llama(args.train_samples, tokenizer, seed=42)
    val_data = generate_ioi_data_llama(args.val_samples, tokenizer, seed=123)
    test_data = generate_ioi_data_llama(args.test_samples, tokenizer, seed=456)

    val_data = filter_dataset_by_model_correctness(
        val_data, full_model, tokenizer, DEVICE, batch_size=args.batch_size
    )
    test_data = filter_dataset_by_model_correctness(
        test_data, full_model, tokenizer, DEVICE, batch_size=args.batch_size
    )

    train_dl = DataLoader(IOIDatasetLlama(train_data, tokenizer, max_length=args.max_seq_length),
                          batch_size=args.batch_size, shuffle=True)
    val_dl = DataLoader(IOIDatasetLlama(val_data, tokenizer, max_length=args.max_seq_length),
                        batch_size=args.batch_size, shuffle=False)
    test_dl = DataLoader(IOIDatasetLlama(test_data, tokenizer, max_length=args.max_seq_length),
                         batch_size=args.batch_size, shuffle=False)
    ...
    # Dense edge count — use Llama config for dimensions
    llama_cfg = full_model.config
    dense_stats = count_dense_edges(
        active_heads, active_mlps,
        num_layers=llama_cfg.num_hidden_layers,
        num_heads_per_layer=llama_cfg.num_attention_heads,
    )
    ...
    print("\n" + "=" * W)
    print("  PHASE 1 SUMMARY — NODE PRUNING (IOI — Llama)")
    print("=" * W)
    ...
    print("\n" + "=" * W)
    print("  PHASE 2 SUMMARY — EDGE PRUNING (IOI — Llama)")
    print("=" * W)
